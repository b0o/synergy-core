...
KeyID InputFilter::KeystrokeCondition::getKey() const { return m_key; }
...
String InputFilter::SwitchToScreenAction::getScreen() const { return m_screen; }
...
InputFilter::KeystrokeAction::~KeystrokeAction() { free(m_keyInfo); }
...
bool InputFilter::KeystrokeAction::isOnPress() const { return m_press; }
...
String InputFilter::KeystrokeAction::format() const {
  ...
  if (m_keyInfo->m_screens[0] == '\0') {
    ...
    return synergy::string::sprintf(
        ...
        strlen(m_keyInfo->m_screens + 1) - 1, m_keyInfo->m_screens + 1);
  }
}
...
InputFilter::MouseButtonAction::~MouseButtonAction() { free(m_buttonInfo); }
...
bool InputFilter::MouseButtonAction::isOnPress() const { return m_press; }
...
InputFilter::Rule::Rule(const Rule &rule) : m_condition(NULL) { copy(rule); }
...
void InputFilter::Rule::replaceAction(Action *adopted, bool onActivation,
                                      UInt32 index) {
  if (adopted == NULL) {
...
InputFilter::~InputFilter() { setPrimaryClient(NULL); }
...
void InputFilter::setPrimaryClient(PrimaryClient *client) {
  ...
  if (m_primaryClient != NULL) {
    for (RuleList::iterator rule = m_ruleList.begin(); rule != m_ruleList.end();
         ++rule) {
...
String InputFilter::format(const String &linePrefix) const {
  String s;
  for (RuleList::const_iterator i = m_ruleList.begin(); i != m_ruleList.end();
       ++i) {
    s += linePrefix;
    s += i->format();
    s += "\n";
