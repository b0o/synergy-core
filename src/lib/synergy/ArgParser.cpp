...
bool ArgParser::parseServerArgs(ServerArgs &args, int argc,
  ...
  for (int i = 1; i < argc; ++i) {
    if (parsePlatformArg(args, argc, argv, i)) {
      ...
      LOG((CLOG_PRINT "%s: unrecognized option `%s'" BYE, args.m_pname, argv[i],
           args.m_pname));
      return false;
    }
  }
...
bool ArgParser::parseClientArgs(ClientArgs &args, int argc,
                                const char *const *argv) {
  setArgsBase(args);
  updateCommonArgs(argv);
...
bool ArgParser::parsePlatformArg(ArgsBase &argsBase, const int &argc,
