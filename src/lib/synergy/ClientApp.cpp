...
ClientApp::ClientApp(IEventQueue *events,
    ...
    : App(events, createTaskBarReceiver, new ClientArgs()), m_client(NULL),
      m_clientScreen(NULL), m_serverAddress(NULL) {}
      ...
      try {
        *m_serverAddress =
        ...
        if (!args().m_restartable || e.getError() == XSocketAddress::kBadPort) {
          LOG((CLOG_PRINT "%s: %s" BYE, args().m_pname, e.what(),
               args().m_pname));
          m_bye(kExitFailed);
        }
      }
...
#define WINAPI_INFO
  ...
  sprintf(
      ...
      "      --yscroll <delta>    defines the vertical scrolling delta, which "
      "is\n"
      "                             120 by default.\n" HELP_COMMON_INFO_2 "\n"
      "* marks defaults.\n"
      "\n"
...
#if WINAPI_MSWINDOWS
  return new synergy::Screen(new MSWindowsScreen(false, args().m_noHooks,
