...
void DaemonApp::mainLoop(bool logToFile) {
  try {
    DAEMON_RUNNING(true);
...
void DaemonApp::handleIpcMessage(const Event &e, void *) {
  ...
  case kIpcCommand: {
    ...
    if (!command.empty()) {
      LOG((CLOG_DEBUG "new command, elevate=%d command=%s", cm->elevate(),
           command.c_str()));
      ...
      bool server =
          argsArray[0].find("synergys") != String::npos ? true : false;
      ...
      if (argBase->m_logFile != NULL) {
        ...
        LOG((CLOG_DEBUG
             "removed log file argument and filename %s from command ",
             logFilename.c_str()));
        LOG((CLOG_DEBUG "new command, elevate=%d command=%s", cm->elevate(),
             command.c_str()));
      } else {
