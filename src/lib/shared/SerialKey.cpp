...
SerialKey::SerialKey(Edition edition)
    : m_userLimit(1), m_warnTime(ULLONG_MAX), m_expireTime(ULLONG_MAX),
      m_edition(edition), m_trial(false) {}

SerialKey::SerialKey(std::string serial)
    : m_userLimit(1), m_warnTime(0), m_expireTime(0), m_edition(kBasic),
      m_trial(true) {
...
bool SerialKey::isExpiring(time_t currentTime) const {
  bool result = false;
...
bool SerialKey::isExpired(time_t currentTime) const {
  bool result = false;
...
bool SerialKey::isTrial() const { return m_trial; }

Edition SerialKey::edition() const { return m_edition; }

std::string SerialKey::editionString() const {
  switch (edition()) {
  case kBasic:
    return "basic";
  case kPro:
...
static std::string hexEncode(std::string const &str) {
  ...
  for (size_t i = 0; i < str.size(); ++i) {
    unsigned c = str[i];
    c %= 256;
    oss << std::setfill('0') << std::hex << std::setw(2) << std::uppercase;
    oss << c;
  }
  return oss.str();
}

std::string SerialKey::toString() const {
  std::ostringstream oss;
  oss << "{";
  if (isTrial()) {
    oss << "v2;trial;";
...
time_t SerialKey::daysLeft(time_t currentTime) const {
...
std::string SerialKey::email() const { return m_email; }

std::string SerialKey::decode(const std::string &serial) {
...
bool SerialKey::parse(std::string plainSerial) {
...
Edition SerialKey::parseEdition(std::string const &editionStr) {
  Edition e = kBasic;
  if (editionStr == "pro") {
    e = kPro;
  }
