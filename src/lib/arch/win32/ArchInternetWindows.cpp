...
String ArchInternetWindows::get(const String &url) {
  WinINetRequest request(url);
  return request.send();
}

String ArchInternetWindows::urlEncode(const String &url) {
  TCHAR buffer[1024];
...
WinINetRequest::WinINetRequest(const String &url)
    : m_session(NULL), m_connect(NULL), m_request(NULL), m_used(false),
      m_url(parseUrl(url)) {}
...
String WinINetRequest::send() {
  ...
  if (!HttpSendRequest(m_request, headers.c_str(), (DWORD)headers.length(),
                       NULL, NULL)) {
    throw XArch(new XArchEvalWindows());
  }
...
void WinINetRequest::connect() {
  m_connect = InternetConnect(m_session, m_url.m_host.c_str(), m_url.m_port,
...
void WinINetRequest::openRequest() {
  m_request = HttpOpenRequest(m_connect, "GET", m_url.m_path.c_str(),
                              HTTP_VERSION, NULL, NULL, m_url.m_flags, NULL);
