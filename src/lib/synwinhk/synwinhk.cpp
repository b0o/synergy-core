...
#if !NO_GRAB_KEYBOARD
static WPARAM makeKeyMsg(UINT virtKey, char c, bool noAltGr) {
...
static bool doKeyboardHookHandler(WPARAM wParam, LPARAM lParam) {
  ...
  PostThreadMessage(
      ...
      wParam | ((c & 0xff) << 8) | ((n & 0xff) << 16) | 0x06000000, lParam);
...
static bool doMouseHookHandler(WPARAM wParam, SInt32 x, SInt32 y, SInt32 data) {
  switch (wParam) {
  case WM_LBUTTONDOWN:
  case WM_MBUTTONDOWN:
  case WM_RBUTTONDOWN:
...
#if _MSC_VER >= 1400
BOOL WINAPI _DllMainCRTStartup(HINSTANCE instance, DWORD reason,
                               LPVOID lpreserved) {
...
#if !NO_GRAB_KEYBOARD
  if (g_keyboardLL == NULL) {
    g_keyboard = SetWindowsHookEx(WH_KEYBOARD, &keyboardHook, g_hinstance, 0);
  }
#endif
...
void setSides(UInt32 sides) { g_zoneSides = sides; }
