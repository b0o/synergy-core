...
MSWindowsWatchdog::MSWindowsWatchdog(bool autoDetectCommand,
    ...
    : m_thread(NULL), m_autoDetectCommand(autoDetectCommand),
      ...
      m_autoElevated(false), m_ready(false) {
...
BOOL MSWindowsWatchdog::doStartProcess(String &command, HANDLE userToken,
  ...
  si.lpDesktop = "winsta0\\Default"; // TODO: maybe this should be \winlogon if
                                     // we have logonui.exe?
...
void MSWindowsWatchdog::outputLoop(void *) {
  ...
  while (m_monitoring) {
    ...
    DWORD bytesRead;
    BOOL success =
        ReadFile(m_stdOutRead, buffer, kOutputBufferSize, &bytesRead, NULL);
