...
XWindowsKeyState::XWindowsKeyState(Display *display, bool useXKB,
                                   IEventQueue *events)
    : KeyState(events), m_display(display),
...
XWindowsKeyState::XWindowsKeyState(Display *display, bool useXKB,
    ...
    : KeyState(events, keyMap), m_display(display),
      m_modifierFromX(ModifiersFromXDefaultSize) {
  init(display, useXKB);
}
...
void XWindowsKeyState::mapKeyToKeycodes(KeyID key,
  ...
  keycodes.clear();
  std::pair<KeyToKeyCodeMap::const_iterator, KeyToKeyCodeMap::const_iterator>
      range = m_keyCodeFromKey.equal_range(key);
  for (KeyToKeyCodeMap::const_iterator i = range.first; i != range.second;
       ++i) {
...
void XWindowsKeyState::updateKeysymMap(synergy::KeyMap &keyMap) {
  ...
  KeySym *allKeysyms = XGetKeyboardMapping(m_display, minKeycode, numKeycodes,
                                           &keysymsPerKeycode);
  ...
  for (int i = 0; i < numKeycodes; ++i) {
    ...
    for (int j = 0; j < maxKeysyms; ++j) {
      ...
      if ((j & 1) != 0) {
        ...
        if (lKeysym != uKeysym && lKeysym == keysyms[j - 1] &&
            uKeysym == keysyms[j]) {
          item.m_required &= ~KeyModifierShift;
  ...
  for (int i = m_xkb->min_key_code; i <= m_xkb->max_key_code; ++i) {
    ...
    for (int group = 0; group < maxNumGroups; ++group) {
      ...
      for (int j = -1; j < type->map_count; ++j) {
        ...
        if ((item.m_required & LockMask) != 0 && j != -1 &&
            type->preserve != NULL &&
...
void XWindowsKeyState::remapKeyModifiers(KeyID id, SInt32 group,
                                         synergy::KeyMap::KeyItem &item,
                                         void *vself) {
