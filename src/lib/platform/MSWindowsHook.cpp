...
MSWindowsHook::MSWindowsHook()
    : m_initFunc(NULL), m_cleanupFunc(NULL), m_setSidesFunc(NULL),
      m_setZoneFunc(NULL), m_setModeFunc(NULL), m_instance(NULL) {}
...
void MSWindowsHook::loadLibrary() {
  ...
  if (m_instance == NULL) {
    LOG((CLOG_ERR "failed to load hook library, %s.dll is missing or invalid",
         g_name));
  ...
  if (m_setSidesFunc == NULL || m_setZoneFunc == NULL ||
    ...
    LOG((CLOG_ERR "failed to load hook function, %s.dll could be out of date",
         g_name));
    throw XScreenOpenFailure();
  }
...
HINSTANCE
MSWindowsHook::getInstance() const { return m_instance; }
