...
OSXKeyState::OSXKeyState(IEventQueue *events) : KeyState(events) { init(); }
...
KeyButton OSXKeyState::mapKeyFromEvent(KeyIDs &ids, KeyModifierMask *maskOut,
  ...
  } else if (CGEventGetIntegerValueField(event, kCGKeyboardEventAutorepeat) ==
             1) {
  ...
  if (layoutValid) {
    ...
    OSStatus status =
        UCKeyTranslate(layout, vkCode & 0xffu, action, (modifiers >> 8) & 0xffu,
...
CGEventFlags OSXKeyState::getModifierStateAsOSXFlags() {
...
void OSXKeyState::postHIDVirtualKey(const UInt8 virtualKeyCode,
                                    const bool postDown) {
  ...
  case s_capsLockVK:
    ...
    kern_return_t kr;
    kr = IOHIDPostEvent(getEventDriver(), NX_FLAGSCHANGED, loc, &event,
                        kNXEventDataVersion, modifiers, true);
    assert(KERN_SUCCESS == kr);
    break;
  ...
  default:
    ...
    kr = IOHIDPostEvent(getEventDriver(), postDown ? NX_KEYDOWN : NX_KEYUP, loc,
...
void OSXKeyState::fakeKey(const Keystroke &keystroke) {
  ...
  case Keystroke::kButton: {
    ...
    LOG((CLOG_DEBUG1 "  button=0x%04x virtualKey=0x%04x keyDown=%s", button,
         virtualKey, keyDown ? "down" : "up"));
...
void OSXKeyState::getKeyMapForSpecialKeys(synergy::KeyMap &keyMap,
                                          SInt32 group) const {
...
bool OSXKeyState::getKeyMap(synergy::KeyMap &keyMap, SInt32 group,
...
bool OSXKeyState::mapSynergyHotKeyToMac(KeyID key, KeyModifierMask mask,
...
void OSXKeyState::handleModifierKeys(void *target, KeyModifierMask oldMask,
...
void OSXKeyState::handleModifierKey(void *target, UInt32 virtualKey, KeyID id,
...
bool OSXKeyState::getGroups(GroupList &groups) const {
  CFIndex n;
...
void OSXKeyState::checkKeyboardLayout() {
...
void OSXKeyState::adjustAltGrModifier(const KeyIDs &ids, KeyModifierMask *mask,
