...
MainWindow::MainWindow(QSettings &settings, AppConfig &appConfig,
    ...
    : m_Settings(settings), m_AppConfig(&appConfig),
      m_LicenseManager(&licenseManager), m_pSynergy(NULL),
      m_SynergyState(synergyDisconnected),
      m_ServerConfig(&m_Settings, 5, 3, m_AppConfig->screenName(), this),
      m_pTempConfigFile(NULL), m_pTrayIcon(NULL), m_pTrayIconMenu(NULL),
      m_AlreadyHidden(false), m_pMenuBar(NULL), m_pMenuFile(NULL),
      m_pMenuEdit(NULL), m_pMenuWindow(NULL), m_pMenuHelp(NULL),
      m_pZeroconfService(NULL), m_pDataDownloader(NULL),
      m_DownloadMessageBox(NULL), m_pCancelButton(NULL),
      m_SuppressAutoConfigWarning(false), m_BonjourInstall(NULL),
      m_SuppressEmptyServerWarning(false), m_ExpectedRunningState(kStopped),
      m_pSslCertificate(NULL), m_ActivationDialogRunning(false) {
  setupUi(this);
  ...
  connect(m_LicenseManager, SIGNAL(beginTrial(bool)), this,
          SLOT(beginTrial(bool)), Qt::QueuedConnection);

  connect(m_LicenseManager, SIGNAL(endTrial(bool)), this, SLOT(endTrial(bool)),
          Qt::QueuedConnection);

  connect(m_AppConfig, SIGNAL(sslToggled(bool)), this, SLOT(sslToggled(bool)),
          Qt::QueuedConnection);
...
void MainWindow::loadSettings() {
  ...
  // groupClientChecked or disabling and/or enabling the right widgets won't
  // automatically work
...
bool MainWindow::autoHide() {
  if ((appConfig().processMode() == Desktop) && appConfig().getAutoHide()) {
    hide();
    return true;
  }
...
void MainWindow::sslToggled(bool enabled) {
  if (enabled) {
  ...
  if (m_pRadioInternalConfig->isChecked()) {
    ...
    if (!m_pTempConfigFile->open()) {
      QMessageBox::critical(this, tr("Cannot write configuration file"),
                            tr("The temporary configuration file required to "
                               "start synergy can not be written."));
      return "";
    }
    ...
    if (!QFile::exists(m_pLineEditConfigFile->text())) {
      if (QMessageBox::warning(
              this, tr("Configuration filename invalid"),
...
void MainWindow::beginTrial(bool isExpiring) {
  ...
  QString expiringNotice("<html><head/><body><p><span style=\""
                         ...
                         "</p></body></html>");
  expiringNotice =
      expiringNotice.arg(daysLeft)
          .arg(
              LicenseManager::getEditionName(m_LicenseManager->activeEdition()))
          .arg((daysLeft == 1) ? "" : "s")
          .arg(QString::fromStdString(m_LicenseManager->serialKey().toString()))
          .arg((daysLeft == 1) ? "s" : "");
...
void MainWindow::on_m_pButtonApply_clicked() { restartSynergy(); }
...
void MainWindow::downloadBonjour() {
  ...
  if (arch == kProcessorArchWin32) {
    ...
    QMessageBox::critical(this, tr("Synergy"),
  ...
  if (m_DownloadMessageBox == NULL) {
    ...
    m_pCancelButton =
        m_DownloadMessageBox->addButton(tr("Cancel"), QMessageBox::RejectRole);
  }
...
int MainWindow::raiseActivationDialog() {
  ...
  connect(&activationDialog, SIGNAL(finished(int)), this,
          SLOT(on_activationDialogFinish()), Qt::QueuedConnection);
  int result = activationDialog.exec();
  m_ActivationDialogRunning = false;
  if (!m_PendingClientNames.empty()) {
    foreach (const QString &name, m_PendingClientNames) { autoAddScreen(name); }
