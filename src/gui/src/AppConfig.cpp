...
AppConfig::AppConfig(QSettings *settings)
    : m_pSettings(settings), m_ScreenName(), m_Port(24800), m_Interface(),
      m_LogLevel(0), m_WizardLastRun(0), m_ProcessMode(DEFAULT_PROCESS_MODE),
      m_AutoConfig(true), m_ElevateMode(defaultElevateMode),
      m_AutoConfigPrompted(false), m_CryptoEnabled(false), m_AutoHide(false),
...
AppConfig::~AppConfig() { saveSettings(); }
...
void AppConfig::loadSettings() {
  m_ScreenName =
      settings().value("screenName", QHostInfo::localHostName()).toString();
  ...
  m_Edition =
      static_cast<Edition>(settings().value("edition", kUnregistered).toInt());
...
void AppConfig::setLastVersion(QString version) { m_lastVersion = version; }
...
void AppConfig::setEdition(Edition e) { m_Edition = e; }
...
void AppConfig::setLastExpiringWarningTime(int t) {
